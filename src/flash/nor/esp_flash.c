...
static const char *esp_stub_err_str(int ret_code)
{
	switch (ret_code) {
		...
		case ESP_STUB_ERR_FLASH_READ_ROM_ERR:
			return "ROM call failed to read data";
...
static int esp_algo_flash_cmd_stub_lib_test_do(struct target *target)
{
	...
	esp_stub_err_print("Called stub_lib_test", ESP_STUB_CMD_TEST1, run.ret_code);
...
static int esp_algo_flash_cmd_apptrace_wr_test_do(struct target *target)
{
	...
	if (run.ret_code != ESP_STUB_OK) {
		esp_stub_err_print("Failed to run apptrace_wr_test", ESP_STUB_CMD_RECV_FROM_HOST, run.ret_code);
		ret = ERROR_FAIL;
	}
	return ret;
}
...
static int esp_algo_flash_cmd_apptrace_rd_test_do(struct target *target)
{
	...
	if (run.ret_code != ESP_STUB_OK) {
		esp_stub_err_print("Failed to run apptrace_rd_test", ESP_STUB_CMD_SEND_TO_HOST, run.ret_code);
		ret = ERROR_FAIL;
	}
...
static COMMAND_HELPER(esp_algo_flash_cmd_flash_info_do, struct target *target)
{
	struct flash_bank *bank;
	int retval = esp_algo_target_to_flash_bank(target, &bank, "flash", false);
	if (retval != ERROR_OK)
	...
	const struct esp_flasher_stub_config *stub_cfg = esp_info->get_stub(bank, ESP_STUB_CMD_FLASH_INFO);
	const uint32_t stack_size = esp_info->stub_log_enabled ?
		stub_cfg->stack_default_sz * 2 : stub_cfg->stack_default_sz;
	...
	init_mem_param(&mp,
		1 /*1st usr arg*/,
		sizeof(struct stub_flash_info) /*size in bytes*/,
		PARAM_IN /* from the stub INTO OOCD */);
	run.mem_args.params = &mp;
	run.mem_args.count = 1;
	...
	if (run.ret_code != ESP_STUB_OK) {
		esp_stub_err_print("Failed to get flash info", ESP_STUB_CMD_FLASH_INFO, run.ret_code);
		destroy_mem_param(&mp);
		return ERROR_FAIL;
	}
	struct stub_flash_info info;
	memcpy(&info, mp.value, sizeof(struct stub_flash_info));
	LOG_INFO("Flash info:\n"
		"\tid: 0x%x, size: %d KB,\n"
...
COMMAND_HANDLER(esp_algo_flash_cmd_flash_info)
{
	return CALL_COMMAND_HANDLER(esp_algo_flash_cmd_flash_info_do, get_current_target(CMD_CTX));
}

static COMMAND_HELPER(esp_algo_flash_cmd_flash_test_do, struct target *target)
{
	uint32_t data_size = 1024 * 1024;
	target_addr_t start_addr = 0x8000;

	if (CMD_ARGC > 2)
		return ERROR_COMMAND_SYNTAX_ERROR;

	if (CMD_ARGC > 0)
		COMMAND_PARSE_NUMBER(u32, CMD_ARGV[0], data_size);
	if (CMD_ARGC > 1)
		COMMAND_PARSE_ADDRESS(CMD_ARGV[1], start_addr);

	LOG_TARGET_INFO(target, "flash test started 0x%x, size %d", (uint32_t)start_addr, data_size);
	...
	const struct esp_flasher_stub_config *stub_cfg = esp_info->get_stub(bank, ESP_STUB_CMD_FLASH_TEST);
	const uint32_t stack_size = esp_info->stub_log_enabled ?
		stub_cfg->stack_default_sz * 2 : stub_cfg->stack_default_sz;
	...
	if (duration_start(&algo_time) != 0) {
		LOG_TARGET_ERROR(target, "Failed to start bench time measurement!");
		return ERROR_FAIL;
	}

	run.stack_size = stack_size;
	run.timeout_ms = 20000;	/* ms */
	ret = esp_info->run_func_image(bank->target,
		&run,
		3,
		/* cmd */
		ESP_STUB_CMD_FLASH_TEST,
		/* start flash addr */
		start_addr,
		/* data size for benching */
		data_size);
	image_close(&run.image.image);
	if (ret != ERROR_OK) {
		LOG_ERROR("Failed to run flasher stub (%d)!", ret);
	...
	if (run.ret_code != ESP_STUB_OK) {
		esp_stub_err_print("Failed to run flash test", ESP_STUB_CMD_FLASH_TEST, run.ret_code);
		return ERROR_FAIL;
	}
	if (duration_measure(&algo_time) != 0) {
		LOG_TARGET_ERROR(target, "Failed to stop bench time measurement!");
		return ERROR_FAIL;
	}
	LOG_TARGET_INFO(target, "PROF: Read %d bytes from 0x%x in %g ms @ %g KB/s",
		data_size,
		(uint32_t)start_addr,
		duration_elapsed(&algo_time) * 1000,
		duration_kbps(&algo_time, data_size));

	return ERROR_OK;
}

COMMAND_HANDLER(esp_algo_flash_cmd_flash_test)
{
	return CALL_COMMAND_HANDLER(esp_algo_flash_cmd_flash_test_do, get_current_target(CMD_CTX));
}
...
const struct command_registration esp_flash_exec_flash_command_handlers[] = {
	...
	{
		.name = "flash_info",
		.handler = esp_algo_flash_cmd_flash_info,
		.mode = COMMAND_ANY,
		.help = "Get SPI Flash info",
		.usage = "",
	},
	{
		.name = "flash_test",
		.handler = esp_algo_flash_cmd_flash_test,
		.mode = COMMAND_ANY,
		.help = "Test SPI Flash speed for read. "
			"data_size - size to read (default is 1M); "
			"flash_addr - address to read (default is 0x8000) ",
		.usage = "[<data_size> [<flash_addr>] ]",
	},
