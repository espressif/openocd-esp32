...
FLASH_BANK_COMMAND_HANDLER(esp32_s2_flash_bank_command)
{
    ...
    int ret = esp_xtensa_flash_init(&esp32_s2_info->esp_xtensa, ESP32_S2_FLASH_SECTOR_SIZE,
                                    xtensa_run_func_image, esp32_s2_is_irom_address, esp32_s2_is_drom_address, &stub_cfg);
    if (ret != ERROR_OK) {
        free(esp32_s2_info);
        return ret;
    }
