...
static int esp_xtensa_dbgstubs_restore(struct target *target)
{
    ...
    LOG_INFO("%s: Restore debug stubs address %x", target_name(target), esp_xtensa->dbg_stubs.base);
...
static int esp_xtensa_handle_target_event(struct target *target, enum target_event event, void *priv)
{
    if (target != priv)
        return ERROR_OK;
    ...
    switch (event) {
        ...
        case TARGET_EVENT_GDB_DETACH:
        {
            ...
            if (target->state != TARGET_HALTED) {
                ...
                if (ret != ERROR_OK) {
                    LOG_ERROR("%s: Failed to halt target to remove flash BPs (%d)!", target_name(target), ret);
                    return ret;
                }
                ret = target_wait_state(target, TARGET_HALTED, 3000);
                if (ret != ERROR_OK) {
                    LOG_ERROR("%s: Failed to wait halted target to remove flash BPs (%d)!", target_name(target), ret);
                    return ret;
                }
            }
            struct esp_xtensa_common *esp_xtensa = target_to_esp_xtensa(target);
            for(size_t slot = 0; slot < ESP_XTENSA_SPECIAL_BREAKPOINTS_MAX_NUM; slot++) {
                struct esp_xtensa_special_breakpoint *spec_bp = &esp_xtensa->spec_brps[slot];
                if(spec_bp->data.oocd_bp != NULL) {
                    int ret = esp_xtensa->spec_brps_ops.breakpoint_remove(target, spec_bp);
                    if (ret != ERROR_OK) {
                        LOG_ERROR("%s: Failed to remove SW flash BP @ " TARGET_ADDR_FMT " (%d)!",
                                target_name(target), spec_bp->data.oocd_bp->address, ret);
                        return ret;
                    }
                }
            }
            memset(esp_xtensa->spec_brps, 0, ESP_XTENSA_SPECIAL_BREAKPOINTS_MAX_NUM*sizeof(struct esp_xtensa_special_breakpoint));
                              ...
                              const struct esp_xtensa_special_breakpoint_ops *spec_brps_ops)
{
    ...
    esp_xtensa->spec_brps = calloc(ESP_XTENSA_SPECIAL_BREAKPOINTS_MAX_NUM, sizeof(struct esp_xtensa_special_breakpoint));
...
bool esp_xtensa_on_halt(struct target *target)
{
    ...
    if (dbg_cause & (DEBUGCAUSE_BI|DEBUGCAUSE_BN)) {
        ...
        res = target_read_memory(target, pc, ESP_XTENSA_SYSCALL_SZ, 1, (uint8_t *)brk_insn_buf);
        if (res != ERROR_OK) {
...
static void esp_xtensa_dbgstubs_info_update(struct target *target)
{
    ...
    for (enum esp_dbg_stub_id i = ESP_DBG_STUB_TABLE_START; i < ESP_DBG_STUB_ENTRY_MAX; i++) {
        ...
        if (esp_xtensa->dbg_stubs.entries[i]) {
            esp_xtensa->dbg_stubs.entries[i] = buf_get_u32((uint8_t *)&esp_xtensa->dbg_stubs.entries[i], 0, 32);
            LOG_DEBUG("New dbg stub %d at %x", esp_xtensa->dbg_stubs.entries_count, esp_xtensa->dbg_stubs.entries[i]);
            esp_xtensa->dbg_stubs.entries_count++;
        }
    }
    if (esp_xtensa->dbg_stubs.entries_count < (ESP_DBG_STUB_ENTRY_MAX-ESP_DBG_STUB_TABLE_START)) {
        LOG_DEBUG("Not full dbg stub table %d of %d", esp_xtensa->dbg_stubs.entries_count, (ESP_DBG_STUB_ENTRY_MAX-ESP_DBG_STUB_TABLE_START));
    ...
    res = target_read_memory(target, esp_xtensa->dbg_stubs.entries[ESP_DBG_STUB_DESC], sizeof(struct esp_dbg_stubs_desc), 1, (uint8_t *)&esp_xtensa->dbg_stubs.desc);
    if (res != ERROR_OK) {
...
static int esp_xtensa_special_breakpoint_add(struct target *target, struct breakpoint *breakpoint)
{
    ...
    for (slot = 0; slot < ESP_XTENSA_SPECIAL_BREAKPOINTS_MAX_NUM; slot++) {
        if (esp_xtensa->spec_brps[slot].data.oocd_bp == NULL || esp_xtensa->spec_brps[slot].data.oocd_bp == breakpoint)
            break;
    }
    ...
    return esp_xtensa->spec_brps_ops.breakpoint_add(target, breakpoint, &esp_xtensa->spec_brps[slot]);
}
...
static int esp_xtensa_special_breakpoint_remove(struct target *target, struct breakpoint *breakpoint)
{
    struct esp_xtensa_common *esp_xtensa = target_to_esp_xtensa(target);
    uint32_t slot;
...
static int esp_xtensa_do_semihosting(struct target *target)
{
    ...
    LOG_DEBUG("Call 0x%x 0x%x 0x%x 0x%x 0x%x %s", a2, a3, a4, a5, a6, esp_xtensa->semihost.basedir ? esp_xtensa->semihost.basedir : "");
    switch (a2) {
    case ESP_SYS_OPEN:
    {
        int mode, base_len = 0;
...
COMMAND_HELPER(esp_xtensa_cmd_flashbootstrap_do, struct esp_xtensa_common *esp_xtensa)
{
    ...
    if (CMD_ARGC < 1) {
        ...
        if (state == FBS_DONTCARE) st = "Don't care";
        else if (state == FBS_TMSLOW) st = "Low (3.3V)";
        else if (state == FBS_TMSHIGH) st = "High (1.8V)";
        else st = "None";
    ...
    else if (!strcasecmp(CMD_ARGV[0], "1.8")) state = FBS_TMSHIGH;
    else if (!strcasecmp(CMD_ARGV[0], "3.3")) state = FBS_TMSLOW;
    else if (!strcasecmp(CMD_ARGV[0], "high")) state = FBS_TMSHIGH;
    else if (!strcasecmp(CMD_ARGV[0], "low")) state = FBS_TMSLOW;
...
{
    if (CMD_ARGC != 1) {
        command_print(CMD_CTX, "Current semihosting base dir: %s", esp_xtensa->semihost.basedir ? esp_xtensa->semihost.basedir : "");
        return ERROR_OK;
    }
...
{
    if (CMD_ARGC < 1) {
        return CALL_COMMAND_HANDLER(esp_xtensa_cmd_flashbootstrap_do, target_to_esp_xtensa(get_current_target(CMD_CTX)));
    }
    return CALL_COMMAND_HANDLER(esp_xtensa_cmd_flashbootstrap_do, target_to_esp_xtensa(get_current_target(CMD_CTX)));
}
...
{
    return CALL_COMMAND_HANDLER(esp_xtensa_cmd_semihost_basedir_do, target_to_esp_xtensa(get_current_target(CMD_CTX)));
}
