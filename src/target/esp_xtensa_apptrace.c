...
// In general tracing goes in the following way. User aplication requests tracing module to send some data by calling esp32_apptrace_buffer_get(),
// moduile allocates necessary buffer in current input trace block. Then user fills received buffer with data and calls esp32_apptrace_buffer_put().
...
// if non-zero then apptrace code entered the critical section and the value is an address of the critical section's exit point
...
static int esp_xtensa_apptrace_data_reverse_read(struct xtensa *xtensa, uint32_t size, uint8_t *buffer, uint8_t *unal_bytes)
{
    ...
    res = xtensa_queue_dbg_reg_write(xtensa, NARADR_TRAXADDR, (xtensa->core_config->trace.mem_sz-rd_sz)/4);
    if (res != ERROR_OK) {
        return res;
    }
    if (size & 0x3UL) {
...
int esp_xtensa_apptrace_ctrl_reg_write(struct target *target, uint32_t block_id, uint32_t len, bool conn, bool data)
{
    struct xtensa *xtensa = target_to_xtensa(target);
    int res = ERROR_OK;
    uint32_t tmp = (conn ? XTENSA_APPTRACE_HOST_CONNECT : 0) | (data ? XTENSA_APPTRACE_HOST_DATA : 0) | XTENSA_APPTRACE_BLOCK_ID(block_id) | XTENSA_APPTRACE_BLOCK_LEN(len);
...
int esp_xtensa_swdbg_activate(struct target *target, int enab)
{
    ...
    xtensa_queue_dbg_reg_write(xtensa, enab ? NARADR_DCRSET : NARADR_DCRCLR, OCDDCR_DEBUGSWACTIVE);
    xtensa_dm_queue_tdi_idle(&xtensa->dbg_mod);
    res = jtag_execute_queue();
    if (res!=ERROR_OK) {
                                                    ...
                                                    uint32_t buf_sz[], const uint8_t *bufs[])
{
    ...
    xtensa_queue_dbg_reg_write(xtensa, NARADR_TRAXADDR, (xtensa->core_config->trace.mem_sz - total_sz) / 4);
    for (uint32_t i = bufs_num; i > 0; i--) {
        ...
        if (cached_bytes) {
            ...
            memcpy(&dword_cache.data8[sizeof(uint32_t) - cached_bytes - bytes_to_cache], cur_buf-bytes_to_cache, bytes_to_cache);
        ...
        for (uint32_t k = bsz; k >= sizeof(uint32_t); k -= sizeof(uint32_t)) {
            res = xtensa_queue_dbg_reg_write(xtensa, NARADR_TRAXDATA, *((uint32_t *)(void *)(cur_buf-sizeof(uint32_t))));
            if (res != ERROR_OK) {
                return res;
            }
        ...
        if (bytes_to_cache > 0) {
            if (bytes_to_cache + cached_bytes >= sizeof(uint32_t)) {
                ...
                memcpy(&dword_cache.data8[sizeof(uint32_t) - to_copy], cur_buf-to_copy, to_copy);
                                                    ...
                                                    uint32_t buf_sz[], const uint8_t *bufs[])
{
    ...
    for (uint32_t i = 0; i < bufs_num; i++) {
        ...
        for (uint32_t k = 0; (k+sizeof(uint32_t)) <= bsz; k += sizeof(uint32_t)) {
            res = xtensa_queue_dbg_reg_write(xtensa, NARADR_TRAXDATA, *((uint32_t *)(void *)cur_buf));
            if (res != ERROR_OK) {
                return res;
            }
            cur_buf += sizeof(uint32_t);
        ...
        if (bytes_to_cache > 0) {
            if (bytes_to_cache + cached_bytes >= sizeof(uint32_t)) {
                memcpy(&dword_cache.data8[cached_bytes], cur_buf, sizeof(uint32_t) - cached_bytes);
                // write full word of cached bytes
                res = xtensa_queue_dbg_reg_write(xtensa, NARADR_TRAXDATA, dword_cache.data32);
                if (res != ERROR_OK) {
                    return res;
                ...
                memcpy(dword_cache.data8, cur_buf, bytes_to_cache + cached_bytes - sizeof(uint32_t));
...
int esp_xtensa_apptrace_buffs_write(struct target *target, uint32_t bufs_num, uint32_t buf_sz[], const uint8_t *bufs[],
                                uint32_t block_id, int ack, int data)
{
    struct xtensa *xtensa = target_to_xtensa(target);
    int res = ERROR_OK;
    uint32_t tmp = (1 << 24) | XTENSA_APPTRACE_HOST_CONNECT | (data ? XTENSA_APPTRACE_HOST_DATA : 0) | XTENSA_APPTRACE_BLOCK_ID(block_id) | XTENSA_APPTRACE_BLOCK_LEN(0);
...
int esp_xtensa_apptrace_usr_block_write(struct target *core_target, uint32_t block_id, const uint8_t *data, uint32_t size)
{
    ...
    // uint32_t buf_sz[] = {sizeof(struct esp_xtensa_apptrace_host2target_hdr), sizeof(test_data1), sizeof(test_data2), sizeof(test_data3), sizeof(test_data4)};
    // const uint8_t *bufs[] = {(const uint8_t *)&hdr, test_data1, (uint8_t *)&test_data2, test_data3, test_data4};
    ...
    return esp_xtensa_apptrace_buffs_write(core_target, sizeof(buf_sz)/sizeof(buf_sz[0]), buf_sz, bufs, block_id, 1/*ack target data*/, 1/*host data*/);
}

int esp_xtensa_sysview_cmds_queue(struct target *core_target, uint8_t *cmds, uint32_t cmds_num, uint32_t block_id)
{
