...
struct xtensa_power_ops {
    /** register read. */
    int (*queue_reg_read)(struct xtensa_debug_module *dm, unsigned reg, uint8_t *data, uint8_t clear);
    /** register write. */
...
struct xtensa_power_status {
    ...
    // TODO: do not need to keep previous status to detect that core or debug module has been reset,
    //       we can clear PWRSTAT_DEBUGWASRESET and PWRSTAT_COREWASRESET after reading will do the job;
...
int xtensa_dm_queue_pwr_reg_read(struct xtensa_debug_module *dm, unsigned reg, uint8_t *data, uint8_t clear);
...
{
    return (!(dm->power_status.prev_stat & PWRSTAT_DEBUGWASRESET) && dm->power_status.stat & PWRSTAT_DEBUGWASRESET);
}

static inline bool xtensa_dm_core_was_reset(struct xtensa_debug_module *dm)
{
    return (!(dm->power_status.prev_stat & PWRSTAT_COREWASRESET) && dm->power_status.stat & PWRSTAT_COREWASRESET);
}
