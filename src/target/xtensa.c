...
static inline const struct xtensa_local_mem_region_config *
xtensa_memory_region_find(const struct xtensa_local_mem_config *mem, target_addr_t address)
{
...
static enum xtensa_reg_id xtensa_windowbase_offset_to_canonical(const enum xtensa_reg_id reg, const int windowbase)
{
    int idx;
...
static enum xtensa_reg_id xtensa_canonical_to_windowbase_offset(const enum xtensa_reg_id reg, const int windowbase)
{
...
{
    if (flags & XT_REGF_NOREAD) return false;
    if ((flags & XT_REGF_COPROC0) && (cpenable & (1 << 0)) == 0) return false;
    return true;
}
...
{
    // TODO: array of size XT_NUM_REGS can be used here to map special register ID to corresponding config option 'enabled' flag
    if (reg_idx >= XT_REG_IDX_LBEG && reg_idx <= XT_REG_IDX_LCOUNT) {
        ...
        return xtensa->core_config->tim_irq.enabled && ((reg_idx-XT_REG_IDX_CCOMPARE0) < xtensa->core_config->tim_irq.comp_num);
...
static int xtensa_write_dirty_registers(struct target *target)
{
    ...
    for (i = 0; i < XT_NUM_REGS; i++) {
        if (reg_list[i].dirty) {
            if (xtensa_regs[i].type == XT_REG_SPECIAL || xtensa_regs[i].type == XT_REG_USER || xtensa_regs[i].type == XT_REG_FR) {
                scratch_reg_dirty = true;
                regval = xtensa_reg_get(target, i);
                LOG_DEBUG("%s: Writing back reg %s val %08X", target_name(target), xtensa_regs[i].name, regval);
                ...
                if (xtensa_regs[i].type == XT_REG_USER) {
                    ...
                    // if (xtensa_user_reg_exists(xtensa, i)) {
                        xtensa_queue_exec_ins(xtensa, XT_INS_WUR(xtensa_regs[i].reg_num, XT_REG_A3));
                    }
                } else if (xtensa_regs[i].type == XT_REG_FR) {
                    if (reg_list[i].exist) {
                    // if (xtensa_fp_reg_exists(xtensa, i)) {
                        xtensa_queue_exec_ins(xtensa, XT_INS_WFR(xtensa_regs[i].reg_num, XT_REG_A3));
                    }
                } else { //SFR
                    if (reg_list[i].exist) {
                    // if (xtensa_special_reg_exists(xtensa, i)) {
                        ...
                        xtensa_queue_exec_ins(xtensa, XT_INS_WSR(reg_num, XT_REG_A3));
                    }
                }
                reg_list[i].dirty = 0;
            }
    ...
    for (i = 0; i < 16; i++) {
        if (reg_list[XT_REG_IDX_A0+i].dirty) {
            ...
            LOG_DEBUG("%s: Writing back reg %s value %08X, num =%i", target_name(target), xtensa_regs[XT_REG_IDX_A0 + i].name, regval, xtensa_regs[XT_REG_IDX_A0 + i].reg_num);
            xtensa_queue_dbg_reg_write(xtensa, NARADR_DDR, regval);
        ...
        for (j = 0; j < 64; j += 16) {
            ...
            for (i = 0; i < 16; i++) {
                if (i+j < xtensa->core_config->aregs_num) {
                    enum xtensa_reg_id realadr = xtensa_windowbase_offset_to_canonical(XT_REG_IDX_AR0+i+j, windowbase);
                    //Write back any dirty un-windowed registers
                    if (reg_list[realadr].dirty) {
                        regval = xtensa_reg_get(target, realadr);
                        LOG_DEBUG("%s: Writing back reg %s value %08X, num =%i", target_name(target), xtensa_regs[realadr].name, regval, xtensa_regs[realadr].reg_num);
                        xtensa_queue_dbg_reg_write(xtensa, NARADR_DDR, regval);
                        xtensa_queue_exec_ins(xtensa, XT_INS_RSR(XT_SR_DDR, xtensa_regs[XT_REG_IDX_AR0+i].reg_num));
...
int xtensa_examine(struct target *target)
{
    ...
    // xtensa_queue_dbg_reg_read(xtensa, NARADR_DSR, dsr_buf);
    // xtensa_queue_dbg_reg_read(xtensa, NARADR_OCDID, id_buf);
...
int xtensa_wakeup(struct target *target)
{
    ...
    // LOG_DEBUG("%s: 0x%x", target_name(target), cmd);
    xtensa_queue_pwr_reg_write(xtensa, DMREG_PWRCTL, cmd);
...
int xtensa_smpbreak_set(struct target *target, uint32_t set)
{
    ...
    uint32_t clear = set ^ (OCDDCR_BREAKINEN|OCDDCR_BREAKOUTEN|OCDDCR_RUNSTALLINEN|OCDDCR_DEBUGMODEOUTEN);
...
int xtensa_core_status_check(struct target *target)
{
    ...
    if (dsr & OCDDSR_EXECEXCEPTION) {
        LOG_ERROR("%s: DSR (%08X) indicates DIR instruction generated an exception!", target_name(target), dsr);
        needclear = 1;
    }
    if (dsr & OCDDSR_EXECOVERRUN) {
        LOG_ERROR("%s: DSR (%08X) indicates DIR instruction generated an overrun!", target_name(target), dsr);
        needclear = 1;
    }
    if (needclear) {
        res = xtensa_dm_core_status_clear(&xtensa->dbg_mod, OCDDSR_EXECEXCEPTION|OCDDSR_EXECOVERRUN);
        if (res != ERROR_OK) {
...
void xtensa_reg_set(struct target *target, enum xtensa_reg_id reg_id, xtensa_reg_val_t value)
{
    struct xtensa *xtensa = target_to_xtensa(target);
    struct reg *reg = &xtensa->core_cache->reg_list[reg_id];
    if (xtensa_reg_get_value(reg) == value) return;
...
int xtensa_assert_reset(struct target *target)
{
    ...
    LOG_DEBUG("%s[%s] coreid=%i, target_number=%i, begin", __func__, target_name(target), target->coreid, target->target_number);
    target->state = TARGET_RESET;
    xtensa_queue_pwr_reg_write(xtensa, DMREG_PWRCTL, PWRCTL_JTAGDEBUGUSE|PWRCTL_DEBUGWAKEUP|PWRCTL_MEMWAKEUP|PWRCTL_COREWAKEUP|PWRCTL_CORERESET);
    xtensa_dm_queue_tdi_idle(&xtensa->dbg_mod);
    res = jtag_execute_queue();
    if (res != ERROR_OK) {
        return res;
...
int xtensa_deassert_reset(struct target *target)
{
    ...
    xtensa_queue_pwr_reg_write(xtensa, DMREG_PWRCTL, PWRCTL_JTAGDEBUGUSE|PWRCTL_DEBUGWAKEUP|PWRCTL_MEMWAKEUP|PWRCTL_COREWAKEUP);
    xtensa_dm_queue_tdi_idle(&xtensa->dbg_mod);
    res = jtag_execute_queue();
    if (res != ERROR_OK) {
        return res;
...
int xtensa_fetch_all_regs(struct target *target)
{
    ...
    for (j = 0; j < XT_AREGS_NUM_MAX; j += 16) {
        ...
        for (i = 0; i < 16; i++) {
            if (i+j < xtensa->core_config->aregs_num) {
                xtensa_queue_exec_ins(xtensa, XT_INS_WSR(XT_SR_DDR, xtensa_regs[XT_REG_IDX_AR0 + i].reg_num));
    ...
    if (xtensa->core_config->coproc) {
        //As the very first thing after AREGS, go grab the CPENABLE registers. It indicates if we can also grab the FP
    ...
    for (i = 0; i < XT_NUM_REGS; i++) {
        if (xtensa_reg_is_readable(xtensa_regs[i].flags, cpenable) && reg_list[i].exist &&
            (xtensa_regs[i].type == XT_REG_SPECIAL || xtensa_regs[i].type == XT_REG_USER || xtensa_regs[i].type == XT_REG_FR)) {
            if (xtensa_regs[i].type == XT_REG_USER) {
...
{
    struct xtensa *xtensa = target_to_xtensa(target);
    *enabled = xtensa->core_config->mmu.itlb_entries_count > 0 || xtensa->core_config->mmu.dtlb_entries_count > 0;
    return ERROR_OK;
}
...
int xtensa_halt(struct target *target)
{
    ...
    if (!(xtensa_dm_core_status_get(&xtensa->dbg_mod) & OCDDSR_STOPPED)) {
        ...
        res = jtag_execute_queue();
        if(res != ERROR_OK) {
            LOG_ERROR("%s: Failed to set OCDDCR_DEBUGINTERRUPT. Can't halt.", target_name(target));
            return res;
        }
    }
             ...
             int debug_execution)
{
    ...
    LOG_DEBUG("%s: current=%d address=" TARGET_ADDR_FMT ", handle_breakpoints=%i, debug_execution=%i)",
    ...
    int handle_breakpoints)
{
    ...
    LOG_DEBUG("%s: current=%d, address="TARGET_ADDR_FMT", handle_breakpoints=%i", target_name(target), current, address, handle_breakpoints);
    ...
    if (xtensa->core_config->debug.icount_sz != 32) {
        LOG_WARNING("%s: stepping for ICOUNT less then 32 bits is not implemented!", target_name(target));
        return ERROR_FAIL;
    }
    ...
    if (xtensa->stepping_isr_mode == XT_STEPPING_ISR_OFF) {
        if (!xtensa->core_config->high_irq.enabled) {
            LOG_WARNING("%s: disabling IRQs while stepping is not implemented w/o high prio IRQs option!", target_name(target));
            return ERROR_FAIL;
        }
        // Increasing the interrupt level to avoid taking interrupts while stepping.
        xtensa_reg_val_t temp_ps = (oldps & ~0xF) | xtensa->core_config->high_irq.excm_level;
    ...
    LOG_DEBUG("%s: oldps=%x, oldpc=%x dbg_cause=%x exc_cause=%x",
                target_name(target), oldps, oldpc, cause, xtensa_reg_get(target, XT_REG_IDX_EXCCAUSE));
    ...
    if (cause & DEBUGCAUSE_DB) {
        ...
        LOG_DEBUG("%s: Single-stepping to get past instruction that triggered the watchpoint...", target_name(target));
        xtensa_reg_set(target, XT_REG_IDX_DEBUGCAUSE, 0); //so we don't recurse into the same routine
        xtensa->core_cache->reg_list[XT_REG_IDX_DEBUGCAUSE].dirty = 0;
    ...
    do {
        ...
        if (res != ERROR_OK) {
            LOG_ERROR("%s: Failed to prepare resume for single step", target_name(target));
            return res;
        }
        res = xtensa_do_resume(target);
        if (res != ERROR_OK) {
            LOG_ERROR("%s: Failed to resume after setting up single step", target_name(target));
            return res;
        }
        ...
        LOG_DEBUG("%s: Finish stepping. dsr=0x%08x", target_name(target), xtensa_dm_core_status_get(&xtensa->dbg_mod));
        if (!xtensa_is_stopped(target)) {
            LOG_WARNING("%s: Timed out waiting for target to finish stepping. dsr=0x%08x",
                target_name(target), xtensa_dm_core_status_get(&xtensa->dbg_mod));
            target->debug_reason = DBG_REASON_NOTHALTED;
            target->state = TARGET_RUNNING;
            return ERROR_FAIL;
        }
        ...
        if (xtensa->core_config->windowed && xtensa->stepping_isr_mode == XT_STEPPING_ISR_OFF &&
        ...
        if (oldpc == cur_pc) {
            LOG_WARNING("%s: Stepping doesn't seem to change PC! dsr=0x%08x", target_name(target), xtensa_dm_core_status_get(&xtensa->dbg_mod));
        }
        else {
...
static inline bool xtensa_memory_op_validate(struct xtensa *xtensa, target_addr_t address, int access)
{
    const struct xtensa_local_mem_region_config *mem_region = xtensa_memory_region_find(&xtensa->core_config->irom, address);
    if (mem_region) {
...
//  LOG_INFO("%s: %s: reading %d bytes from addr %08X", target_name(target), __FUNCTION__, size*count, address);
//  LOG_INFO("Converted to aligned addresses: read from %08X to %08X", addrstart_al, addrend_al);
    ...
    if (!xtensa_memory_op_validate(xtensa, address, XT_MEM_ACCESS_READ) && !xtensa->permissive_mode) {
    ...
    if (addrstart_al == address && addrend_al == address + (size*count)) {
        ...
        if (!albuff) {
            LOG_ERROR("%s: Out of memory allocating %ld bytes!", __FUNCTION__, (long)(addrend_al-addrstart_al));
    ...
    if (res != ERROR_OK) {
        LOG_WARNING("%s: Failed reading %d bytes at address "TARGET_ADDR_FMT, target_name(target), count*size, address);
    }
                        ...
                        const uint8_t *buffer)
{
    ...
    if (!xtensa_memory_op_validate(xtensa, address, XT_MEM_ACCESS_WRITE) && !xtensa->permissive_mode) {
        LOG_DEBUG("address "TARGET_ADDR_FMT" not writable", address);
        return ERROR_FAIL;
    }

//  LOG_INFO("%s: %s: writing %d bytes to addr %08X", target_name(target), __FUNCTION__, size*count, address);
//  LOG_INFO("al start %x al end %x", addrstart_al, addrend_al);
    ...
    if (albuff != buffer) {
        ...
        if ((address + (size*count)) & 3) {
            ...
            xtensa_queue_exec_ins(xtensa, XT_INS_RSR(XT_SR_DDR, XT_REG_A3));
            xtensa_queue_exec_ins(xtensa, XT_INS_LDDR32P(XT_REG_A3));
            xtensa_queue_dbg_reg_read(xtensa, NARADR_DDR, &albuff[addrend_al-addrstart_al-4]);
        }
        //Grab bytes
        res = jtag_execute_queue();
        xtensa_core_status_check(target);
    ...
    if (res!=ERROR_OK) {
        LOG_WARNING("%s: Failed writing %d bytes at address "TARGET_ADDR_FMT, target_name(target), count*size, address);
    }
    if (albuff != buffer) {
        free(albuff);
    }
    ...
    if (xtensa->trace_active) {
        ...
        if (res == ERROR_OK) {
            if (!(trace_status.stat & TRAXSTAT_TRACT)) {
                ...
                if (trace_status.stat & TRAXSTAT_PCMTG) LOG_INFO("%s: Trace stop triggered by PC match", target_name(target));
                if (trace_status.stat &TRAXSTAT_PTITG) LOG_INFO("%s: Trace stop triggered by Processor Trigger Input", target_name(target));
                if (trace_status.stat & TRAXSTAT_CTITG) LOG_INFO("%s: Trace stop triggered by Cross-trigger Input", target_name(target));
...
int xtensa_poll(struct target *target)
{
    ...
    //LOG_DEBUG("%s: 0x%x %d", target_name(target), xtensa->dbg_mod.core_status.dsr, xtensa->dbg_mod.core_status.dsr & OCDDSR_STOPPED);
    if (xtensa_dm_power_status_get(&xtensa->dbg_mod) & PWRSTAT_COREWASRESET) {
        ...
        if(target->state != TARGET_HALTED) {
            ...
            LOG_DEBUG("%s: Target halted, pc=0x%08X, debug_reason=%08x, oldstate=%08x", target_name(target),
                xtensa_reg_get(target, XT_REG_IDX_PC), target->debug_reason, oldstate);
            LOG_DEBUG("%s: Halt reason=0x%08X, exc_cause=%d, dsr=0x%08x", target_name(target),
                halt_cause, xtensa_reg_get(target, XT_REG_IDX_EXCCAUSE), xtensa->dbg_mod.core_status.dsr);
            LOG_INFO("%s: Target halted, PC=0x%08X, debug_reason=%08x", target_name(target),
                xtensa_reg_get(target, XT_REG_IDX_PC), target->debug_reason);
            xtensa_dm_core_status_clear(&xtensa->dbg_mod, OCDDSR_DEBUGPENDBREAK|OCDDSR_DEBUGINTBREAK|OCDDSR_DEBUGPENDHOST|OCDDSR_DEBUGINTHOST);
            ...
            if (oldstate == TARGET_DEBUG_RUNNING) {
                ...
                need_resume = xtensa->chip_ops != NULL && xtensa->chip_ops->on_halt != NULL ? xtensa->chip_ops->on_halt(target) : false;
                // in case of semihosting call we will resume automatically a bit later, so do not confuse GDB
                if (!need_resume) {
                    target_call_event_callbacks(target, TARGET_EVENT_HALTED);
                }
            }
...
static int xtensa_sw_breakpoint_add(struct target *target, struct breakpoint *breakpoint, struct xtensa_sw_breakpoint *sw_bp)
{
    ...
    if (ret != ERROR_OK) {
        LOG_ERROR("%s: Failed to read original instruction (%d)!", target_name(target), ret);
        return ret;
    }
    ...
    if (ret != ERROR_OK) {
        LOG_ERROR("%s: Failed to write breakpoint instruction (%d)!", target_name(target), ret);
        return ret;
    }
...
int xtensa_breakpoint_add(struct target *target, struct breakpoint *breakpoint)
{
    ...
    if (breakpoint->type == BKPT_SOFT) {
        for (slot = 0; slot < XT_SW_BREAKPOINTS_MAX_NUM; slot++) {
            if (xtensa->sw_brps[slot].oocd_bp == NULL || xtensa->sw_brps[slot].oocd_bp == breakpoint)
                break;
        }
        ...
        LOG_DEBUG("%s: placed SW breakpoint %u @ " TARGET_ADDR_FMT, target_name(target), slot, breakpoint->address);
        return ERROR_OK;
    }
    ...
    LOG_DEBUG("%s: placed HW breakpoint @ " TARGET_ADDR_FMT, target_name(target), breakpoint->address);
...
int xtensa_breakpoint_remove(struct target *target, struct breakpoint *breakpoint)
{
    ...
    if (breakpoint->type == BKPT_SOFT) {
        ...
        if (slot == XT_SW_BREAKPOINTS_MAX_NUM) {
            LOG_WARNING("%s: Max SW breakpoints slot reached, slot=%u!", target_name(target), slot);
            return ERROR_TARGET_RESOURCE_NOT_AVAILABLE;
        }
        int ret = xtensa_sw_breakpoint_remove(target, &xtensa->sw_brps[slot]);
        if (ret != ERROR_OK) {
            LOG_ERROR("%s: Failed to remove SW breakpoint (%d)!", target_name(target), ret);
            return ret;
        }
        LOG_DEBUG("%s: cleared SW breakpoint %u @ " TARGET_ADDR_FMT, target_name(target), slot, breakpoint->address);
        return ERROR_OK;
    }
    ...
    LOG_DEBUG("%s: cleared HW breakpoint %u @ " TARGET_ADDR_FMT, target_name(target), slot, breakpoint->address);
    return ERROR_OK;
}
...
int xtensa_watchpoint_add(struct target *target, struct watchpoint *watchpoint)
{
    ...
    if (dbreakcval == 0xaa) {
        LOG_WARNING("%s: Watchpoint with length %d on address "TARGET_ADDR_FMT" not supported by hardware.", target_name(target), watchpoint->length, watchpoint->address);
        return ERROR_TARGET_RESOURCE_NOT_AVAILABLE;
    }
    ...
    LOG_DEBUG("%s: placed HW watchpoint @ " TARGET_ADDR_FMT, target_name(target), watchpoint->address);
    return ERROR_OK;
}
...
int xtensa_watchpoint_remove(struct target *target, struct watchpoint *watchpoint)
{
    ...
    LOG_DEBUG("%s: cleared HW watchpoint @ " TARGET_ADDR_FMT, target_name(target), watchpoint->address);
    return ERROR_OK;
}
    ...
    void *arch_info)
{
    ...
    if (retval != ERROR_OK || target->state != TARGET_HALTED) {
        ...
        LOG_ERROR("xtensa_wait_algorithm: not halted %d, pc 0x%x, ps 0x%x", retval,
            xtensa_reg_get(target, XT_REG_IDX_PC), xtensa_reg_get(target, XT_REG_IDX_PS));
    ...
    for (int i = xtensa->core_cache->num_regs - 1; i >= 0; i--) {
        ...
        if (i == XT_REG_IDX_DEBUGCAUSE) {
            ...
            LOG_DEBUG("Skip restoring register %s: 0x%x -> 0x%8.8" PRIx32,
                    xtensa->core_cache->reg_list[i].name, regvalue, algorithm_info->context[i]);
...
{
    return CALL_COMMAND_HANDLER(xtensa_cmd_permissive_mode_do, target_to_xtensa(get_current_target(CMD_CTX)));
}
...
{
    return CALL_COMMAND_HANDLER(xtensa_cmd_perfmon_enable_do, target_to_xtensa(get_current_target(CMD_CTX)));
}
...
{
    return CALL_COMMAND_HANDLER(xtensa_cmd_perfmon_dump_do, target_to_xtensa(get_current_target(CMD_CTX)));
}
...
COMMAND_HELPER(xtensa_cmd_mask_interrupts_do, struct xtensa *xtensa)
{
    ...
    if (CMD_ARGC < 1) {
        ...
        if (state == XT_STEPPING_ISR_ON) st = "OFF";
        else if (state == XT_STEPPING_ISR_OFF) st = "ON";
        else st = "UNKNOWN";
        command_print(CMD_CTX, "Current ISR step mode: %s", st);
        return ERROR_OK;
    }
    // Masking is ON -> interrupts during stepping are OFF, and vice versa
    if (!strcasecmp(CMD_ARGV[0], "off")) state = XT_STEPPING_ISR_ON;
    else if (!strcasecmp(CMD_ARGV[0], "on")) state = XT_STEPPING_ISR_OFF;
...
{
    return CALL_COMMAND_HANDLER(xtensa_cmd_mask_interrupts_do, target_to_xtensa(get_current_target(CMD_CTX)));
}
...
COMMAND_HELPER(xtensa_cmd_tracestart_do, struct xtensa *xtensa)
{
    ...
    for (i = 0; i < CMD_ARGC; i++) {
        if ((!strcasecmp(CMD_ARGV[i], "pc")) && CMD_ARGC > i) {
            ...
            cfg.stopmask = 0;
            if (*e == '/') cfg.stopmask = strtol(e, NULL, 0);
...
{
    return CALL_COMMAND_HANDLER(xtensa_cmd_tracestart_do, target_to_xtensa(get_current_target(CMD_CTX)));
}
...
{
    return CALL_COMMAND_HANDLER(xtensa_cmd_tracestop_do, target_to_xtensa(get_current_target(CMD_CTX)));
}
...
COMMAND_HELPER(xtensa_cmd_tracedump_do, struct xtensa *xtensa, const char *fname)
{
    ...
    if ((trace_config.addr & ((TRAXADDR_TWRAP_MASK << TRAXADDR_TWRAP_SHIFT) | TRAXADDR_TWSAT)) == 0) {
...
COMMAND_HANDLER(xtensa_cmd_tracedump)
{
    ...
    return CALL_COMMAND_HANDLER(xtensa_cmd_tracedump_do, target_to_xtensa(get_current_target(CMD_CTX)), CMD_ARGV[0]);
}
