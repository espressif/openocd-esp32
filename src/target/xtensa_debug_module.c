...
int xtensa_dm_trace_start(struct xtensa_debug_module *dm, struct xtensa_trace_start_config *cfg)
{
    ...
    if (cfg->stopmask != (uint32_t)-1) {
        dm->dbg_ops->queue_reg_write(dm, NARADR_PCMATCHCTRL, (cfg->stopmask << PCMATCHCTRL_PCML_SHIFT));
    ...
    dm->dbg_ops->queue_reg_write(dm, NARADR_TRAXCTRL,
        TRAXCTRL_TREN | ((cfg->stopmask != (uint32_t)-1) ? TRAXCTRL_PCMEN : 0) | TRAXCTRL_TMEN |
