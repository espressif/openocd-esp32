...
static inline int esp_xtensa_apptrace_data_len_read(struct target *target, uint32_t *block_id, uint32_t *len)
{
...
{
    struct xtensa *xtensa = target_to_xtensa(target);
    return (xtensa->core_config->trace.mem_sz - sizeof(struct esp_xtensa_apptrace_host2target_hdr));
}
