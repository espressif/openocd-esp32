...
static int xtensa_stub_load(struct target *target, struct xtensa_algo_image *algo_image, struct xtensa_stub *stub, uint32_t stack_size)
{
...
static int xtensa_stub_fill_stack(struct target *target, uint32_t stack_addr, uint32_t stack_size, uint32_t sz)
{
    uint8_t buf[256];
...
static int xtensa_stub_check_stack(struct target *target, uint32_t stack_addr, uint32_t stack_size, uint32_t sz)
{
    int retval = ERROR_OK;
    uint8_t buf[256];
                        ...
                        struct xtensa_algo_run_data *run)
{
    ...
    if (run->mem_args.count > 0) {
        ...
        for (uint32_t i = 0; i < run->mem_args.count; i++) {
            ...
            if (image) {
                ...
                retval = xtensa_run_onboard_func(target, &alloc_run, run->on_board.alloc_func, 1, run->mem_args.params[i].size);
                if (retval != ERROR_OK) {
            ...
            if (usr_param_num != UINT_MAX) { // if we need update some register param with to mem param addr
                buf_set_u32(run->priv.stub.reg_params[XTENSA_STUB_ARGS_FUNC_START+usr_param_num].value,
    ...
    retval = xtensa_stub_fill_stack(target, run->priv.stub.stack_addr - run->stack_size, run->stack_size, XTENSA_STUB_STACK_DEBUG);
    if (retval != ERROR_OK) {
        LOG_ERROR("Failed to init stub stack (%d)!", retval);
        goto _cleanup;
    }
...
#if XTENSA_STUB_STACK_DEBUG
    retval = xtensa_stub_check_stack(target, run->priv.stub.stack_addr - run->stack_size, run->stack_size, XTENSA_STUB_STACK_DEBUG);
    if (retval != ERROR_OK) {
        ...
        for (uint32_t i = 0; i < run->mem_args.count; i++) {
            if (mem_handles[i]) {
                if (image) {
                    ...
                    retval = xtensa_run_onboard_func(target, &free_run, run->on_board.free_func, 1, mem_handles[i]);
                    if (retval != ERROR_OK) {
...
int xtensa_run_algorithm_image(struct target *target, struct xtensa_algo_run_data *run, struct xtensa_algo_image *algo_image)
{
...
int xtensa_run_algorithm_onboard(struct target *target, struct xtensa_algo_run_data *run, uint32_t entry)
{
    int res;
    struct xtensa *xtensa = target_to_xtensa(target);
    struct xtensa_stub_tramp stub_tramp;
    ...
    if (run->stack_size > run->on_board.min_stack_size) {
        ...
        alloc_run.on_board.code_buf_size = run->on_board.code_buf_size;
        alloc_run.on_board.code_buf_addr = run->on_board.code_buf_addr;
        res = xtensa_run_onboard_func(target, &alloc_run, run->on_board.alloc_func, 1, run->stack_size);
        if (res != ERROR_OK) {
    ...
    if (run->stack_size > run->on_board.min_stack_size) {
        ...
        free_run.on_board.code_buf_size = run->on_board.code_buf_size;
        free_run.on_board.code_buf_addr = run->on_board.code_buf_addr;
        res = xtensa_run_onboard_func(target, &free_run, run->on_board.free_func, 1, run->priv.stub.stack_addr - run->stack_size);
        if (res != ERROR_OK) {
...
static int xtensa_run_do(struct target *target, struct xtensa_algo_run_data *run, void *algo_arg, uint32_t num_args, va_list ap)
{
    char *arg_regs[] = {"a3", "a4", "a5", "a6"};

    init_reg_param(&run->priv.stub.reg_params[XTENSA_STUB_ARGS_FUNC_START+0], "a2", 32, PARAM_IN_OUT);
    if (num_args > 0) {
        uint32_t arg = va_arg(ap, uint32_t);
        buf_set_u32(run->priv.stub.reg_params[XTENSA_STUB_ARGS_FUNC_START+0].value, 0, 32, arg);
        LOG_DEBUG("Set arg[0] = %d", arg);
    } else {
        buf_set_u32(run->priv.stub.reg_params[XTENSA_STUB_ARGS_FUNC_START+0].value, 0, 32, 0);
    }
    for (uint32_t i = 1; i < num_args; i++) {
        uint32_t arg = va_arg(ap, uint32_t);
        init_reg_param(&run->priv.stub.reg_params[XTENSA_STUB_ARGS_FUNC_START+i], arg_regs[i-1], 32, PARAM_OUT);
        buf_set_u32(run->priv.stub.reg_params[XTENSA_STUB_ARGS_FUNC_START+i].value, 0, 32, arg);
    ...
    if (retval != ERROR_OK) {
        ...
        run->ret_code = buf_get_u32(run->priv.stub.reg_params[XTENSA_STUB_ARGS_FUNC_START+0].value, 0, 32);
