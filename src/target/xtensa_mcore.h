...
struct xtensa_core_ops {
    int (*core_init_arch_info)(struct target *target, struct target *chip_target, const struct xtensa_config *xtensa_cfg,
    ...
    // 'special' means that breakpoint is neither HW nor SW and requires some unusual (chip-specific) handling (running flasher or smth like this).
...
struct xtensa_mcore_common {
    ...
    // Pointer to array of multi-core support operations for cores. Array length is `configured_cores_num`.
...
static inline struct xtensa_mcore_common *
target_to_xtensa_mcore(struct target *target)
{
    return (struct xtensa_mcore_common *)target->arch_info;
}

static inline void xtensa_mcore_stepping_isr_mode_set(struct target *target, enum xtensa_stepping_isr_mode mode)
{
