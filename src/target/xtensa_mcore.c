...
int xtensa_mcore_halt(struct target *target)
{
    ...
    // when SMP breakINOUT is enabled we can halt just one core, the second will be halted via BreakInOut circuit
    if ((xtensa_mcore->smp_break & (OCDDCR_BREAKINEN|OCDDCR_BREAKOUTEN)) == (OCDDCR_BREAKINEN|OCDDCR_BREAKOUTEN)) {
        struct target *sub_target = &xtensa_mcore->cores_targets[xtensa_mcore->active_core];
        res = sub_target->type->halt(sub_target);
        if (res != ERROR_OK) {
            LOG_ERROR("%s.%s: Failed to halt!", target_name(target), target_name(sub_target));
            return res;
        }
    } else for (size_t i = 0; i < xtensa_mcore->configured_cores_num; i++) {
        struct target *sub_target = &xtensa_mcore->cores_targets[i];
        res = sub_target->type->halt(sub_target);
        if (res != ERROR_OK) {
            LOG_ERROR("%s.%s: Failed to halt!", target_name(target), target_name(sub_target));
             ...
             int debug_execution)
{
    ...
    // we break resume process into two phases in order to resume both cores as simultaneously as we can
    // at the first stage we write all the necessary data to the cores (HW breakpoints/watchpoints, dirty registers etc.)
    ...
    res = xtensa_mcore->cores_ops[xtensa_mcore->active_core]->core_prepare_resume(sub_target, current, address, handle_breakpoints, debug_execution);
    if (res != ERROR_OK) {
        LOG_ERROR("%s.%s: Failed to prepare for resume on active core!", target_name(target), target_name(sub_target));
        return res;
    }
    for (size_t i = 0; i < xtensa_mcore->configured_cores_num; i++) {
        ...
        // This operation required to clear state of non-active cores, because core_prepare_resume() on active can step over watchpoint and
        ...
        res = xtensa_mcore->cores_ops[i]->core_prepare_resume(sub_target, i == xtensa_mcore->active_core ? current : 1, address, handle_breakpoints, debug_execution);
        if (res != ERROR_OK) {
            LOG_ERROR("%s.%s: Failed to prepare for resume!", target_name(target), target_name(sub_target));
            return res;
        }
    }
    for (size_t i = 0; i < xtensa_mcore->configured_cores_num; i++) {
        ...
        if (res != ERROR_OK) {
            LOG_ERROR("%s.%s: Failed to resume!", target_name(target), target_name(sub_target));
            return res;
        }
    ...
    int handle_breakpoints)
{
    ...
    int retval = xtensa_mcore->cores_ops[xtensa_mcore->active_core]->core_do_step(sub_target, current, address, handle_breakpoints);
    if (retval != ERROR_OK) {
    ...
    for (size_t i = 0; i < xtensa_mcore->configured_cores_num; i++) {
        if (i != xtensa_mcore->active_core) {
            xtensa_core_status_clear(&xtensa_mcore->cores_targets[i], OCDDSR_DEBUGPENDBREAK|OCDDSR_DEBUGINTBREAK);
        }
    }
...
int xtensa_mcore_checksum_memory(struct target *target, target_addr_t address, uint32_t count, uint32_t *checksum)
{
    struct xtensa_mcore_common *xtensa_mcore = target_to_xtensa_mcore(target);
    struct target *sub_target = &xtensa_mcore->cores_targets[xtensa_mcore->active_core];
...
int xtensa_mcore_target_init(struct command_context *cmd_ctx, struct target *target)
{
    ...
    for (int i = 0; i < xtensa_mcore->configured_cores_num; i++) {
        ...
        // it is safe to have the same woking areas on both CPUs, because algorithm can be run only on one CPU at the same time
        memcpy(&sub_target->working_area_cfg, &target->working_area_cfg, sizeof(sub_target->working_area_cfg));
        memcpy(&sub_target->alt_working_area_cfg, &target->alt_working_area_cfg, sizeof(sub_target->alt_working_area_cfg));
                              ...
                              void *cores_privs[])
{
    ...
    xtensa_mcore->cores_targets = calloc(xtensa_mcore->configured_cores_num, sizeof(struct target));
    if (xtensa_mcore->cores_targets == NULL) {
        LOG_ERROR("Failed to alloc memory for sub-targets!");
        return ERROR_FAIL;
    }
    xtensa_mcore->cores_ops = calloc(xtensa_mcore->configured_cores_num, sizeof(struct xtensa_core_ops));
    if (xtensa_mcore->cores_ops == NULL) {
        LOG_ERROR("Failed to alloc memory for cores ops!");
        return ERROR_FAIL;
    }
    memcpy(xtensa_mcore->cores_ops, mcores_ops, xtensa_mcore->configured_cores_num*sizeof(struct xtensa_core_ops *));
    for (uint32_t i = 0; i < xtensa_mcore->configured_cores_num; i++) {
        ...
        int ret = mcores_ops[i]->core_init_arch_info(subtarget, target, xtensa_cfgs[i], &dm_cfgs[i], NULL, cores_privs[i]);
        if (ret != ERROR_OK) {
            LOG_ERROR("Failed to alloc memory for arch info!");
            free(subtarget->cmd_name); //TODO: free cmd_name for already initialized subtargets
            free(xtensa_mcore->cores_ops);
            free(xtensa_mcore->cores_targets);
            return ret;
        }
...
int xtensa_mcore_poll(struct target *target)
{
    ...
    for (size_t i = 0; i < xtensa_mcore->configured_cores_num; i++) {
        struct xtensa *xtensa = target_to_xtensa(&xtensa_mcore->cores_targets[i]);
        res = xtensa_dm_power_status_read(&xtensa->dbg_mod, PWRSTAT_DEBUGWASRESET|PWRSTAT_COREWASRESET);
    ...
    if (common_core_stat & OCDDSR_STOPPED) {
        ...
        if (target->state == TARGET_DEBUG_RUNNING) {
            ...
            if (enab_cores_num > 1) {
                for (size_t i = 0; i < enab_cores_num; i++) {
                    struct xtensa *xtensa = target_to_xtensa(&xtensa_mcore->cores_targets[i]);
                    if (!(xtensa_dm_core_status_get(&xtensa->dbg_mod) & (OCDDSR_STOPPED|OCDDSR_RUNSTALLSAMPLE))) {
                        algo_stopped = false; // algo is not stopped if any of the core is running
                        break;
                    }
                }
            } else {
                struct xtensa *xtensa = target_to_xtensa(&xtensa_mcore->cores_targets[0]);
                algo_stopped = xtensa_dm_core_status_get(&xtensa->dbg_mod) & OCDDSR_STOPPED;
            }
        }
        if ((target->state != TARGET_HALTED && target->state != TARGET_DEBUG_RUNNING) || algo_stopped) {
            ...
            for (size_t k = xtensa_mcore->configured_cores_num; k > 0; k--) {
                ...
                xtensa_reg_val_t halt_cause = xtensa_reg_get(sub_target, XT_REG_IDX_DEBUGCAUSE);
                struct xtensa *xtensa = target_to_xtensa(sub_target);
                if ((halt_cause & DEBUGCAUSE_DI) &&
                    ((xtensa_dm_core_status_get(&xtensa->dbg_mod)&(OCDDSR_DEBUGPENDHOST|OCDDSR_DEBUGINTHOST)) != 0))
                {
            ...
            for (size_t k = xtensa_mcore->configured_cores_num; k > 0; k--) {
                xtensa_reg_val_t halt_cause = xtensa_reg_get(&xtensa_mcore->cores_targets[k-1], XT_REG_IDX_DEBUGCAUSE);
            ...
            if (target->debug_reason == DBG_REASON_DBGRQ) {
                struct xtensa *xtensa = target_to_xtensa(&xtensa_mcore->cores_targets[xtensa_mcore->active_core]);
                if (xtensa_dm_core_status_get(&xtensa->dbg_mod) & OCDDSR_RUNSTALLSAMPLE) {
                    LOG_DEBUG("Received debug request on stalled active core %d. Switch active core.", (int)xtensa_mcore->active_core);
                    ...
                    for (k = 0; k < xtensa_mcore->configured_cores_num; k++) {
                        ...
                        xtensa = target_to_xtensa(&xtensa_mcore->cores_targets[k]);
                        if ((xtensa_dm_core_status_get(&xtensa->dbg_mod) & OCDDSR_RUNSTALLSAMPLE) == 0) {
                            xtensa_mcore->active_core = k;
                            break;
                        }
                    }
                    if (k == xtensa_mcore->configured_cores_num) {
                        LOG_WARNING("Failed to switch stalled active core %d. All cores are stalled!", (int)xtensa_mcore->active_core);
                    }
                }
            }
            for (size_t i = 0; i < xtensa_mcore->configured_cores_num; i++) {
                struct target *sub_target = &xtensa_mcore->cores_targets[i];
                struct xtensa *xtensa = target_to_xtensa(sub_target);
                LOG_DEBUG("%s.%s: Target halted, pc=0x%08X, debug_reason=%08x, oldstate=%08x, active=%s", target_name(target), target_name(sub_target),
                    xtensa_reg_get(sub_target, XT_REG_IDX_PC), target->debug_reason, oldstate, (i == xtensa_mcore->active_core) ? "true" : "false");
                LOG_DEBUG("%s.%s: Halt reason=0x%08X, exc_cause=%d, dsr=0x%08x", target_name(target), target_name(sub_target),
                    xtensa_reg_get(sub_target, XT_REG_IDX_DEBUGCAUSE), xtensa_reg_get(sub_target, XT_REG_IDX_EXCCAUSE), xtensa_dm_core_status_get(&xtensa->dbg_mod));
                LOG_INFO("Target halted. CPU%u: PC=0x%08X %s", (uint32_t)i,
                    xtensa_reg_get(sub_target, XT_REG_IDX_PC), xtensa_mcore->active_core == i ? "(active)" : "");
            ...
            target->reg_cache = xtensa_mcore->cores_targets[xtensa_mcore->active_core].reg_cache;
            ...
            if (oldstate == TARGET_DEBUG_RUNNING) {
                ...
                need_resume = xtensa_mcore->chip_ops->on_halt != NULL ? xtensa_mcore->chip_ops->on_halt(target) : false;
        ...
        if (target->state != TARGET_RUNNING && target->state != TARGET_DEBUG_RUNNING) {
            LOG_DEBUG("%s: Target %s -> RUNNING", target_name(target), target_state_name(target));
            target->state = TARGET_RUNNING;
            for (size_t i = 0; i < xtensa_mcore->configured_cores_num; i++) {
...
int xtensa_mcore_breakpoint_add(struct target *target, struct breakpoint *breakpoint)
{
    ...
    for (size_t i = 0; i < xtensa_mcore->configured_cores_num; i++)
    {
        ...
        if (breakpoint->type == BKPT_SOFT || xtensa_mcore->cores_ops[i]->core_is_special_breakpoint(sub_target, breakpoint)) {
...
int xtensa_mcore_breakpoint_remove(struct target *target, struct breakpoint *breakpoint)
{
    ...
    {
        struct target *sub_target = &xtensa_mcore->cores_targets[i];
        bool is_special_breakpoint = xtensa_mcore->cores_ops[i]->core_is_special_breakpoint(sub_target, breakpoint);
...
int xtensa_mcore_run_func_image(struct target *target, struct xtensa_algo_run_data *run, struct xtensa_algo_image *image, uint32_t num_args, ...)
{
    struct xtensa_mcore_common *xtensa_mcore = target_to_xtensa_mcore(target);
    struct target *sub_target = &xtensa_mcore->cores_targets[xtensa_mcore->active_core];
    va_list ap;
...
size_t xtensa_mcore_get_enabled_cores_count(struct target *target)
{
    ...
    return xtensa_mcore->cores_num ? xtensa_mcore->cores_num : xtensa_mcore->configured_cores_num;
}
...
{
    struct xtensa_mcore_common *xtensa_mcore = target_to_xtensa_mcore(get_current_target(CMD_CTX));

    for (int i = 0; i < xtensa_mcore->configured_cores_num; i++) {
        int ret = CALL_COMMAND_HANDLER(xtensa_cmd_permissive_mode_do, target_to_xtensa(&xtensa_mcore->cores_targets[i]));
        if (ret != ERROR_OK) {
            return ret;
        }
    }
...
COMMAND_HANDLER(xtensa_mcore_cmd_mask_interrupts)
{
    ...
    if (CMD_ARGC < 1) {
        return CALL_COMMAND_HANDLER(xtensa_cmd_mask_interrupts_do, target_to_xtensa(&xtensa_mcore->cores_targets[0]));
    }
    for (int i = 0; i < xtensa_mcore->configured_cores_num; i++) {
        int res = CALL_COMMAND_HANDLER(xtensa_cmd_mask_interrupts_do, target_to_xtensa(&xtensa_mcore->cores_targets[i]));
        if (res != ERROR_OK) {
            return res;
        }
    }
...
COMMAND_HANDLER(xtensa_mcore_cmd_perfmon_enable)
{
    ...
    for (int i = 0; i < xtensa_mcore->configured_cores_num; i++) {
        int res = CALL_COMMAND_HANDLER(xtensa_cmd_perfmon_enable_do, target_to_xtensa(&xtensa_mcore->cores_targets[i]));
        if (res != ERROR_OK) {
            return res;
        }
    }
...
COMMAND_HANDLER(xtensa_mcore_cmd_perfmon_dump)
{
    ...
    for (int i = 0; i < xtensa_mcore->configured_cores_num; i++) {
        LOG_INFO("CPU%d:", i);
        int res = CALL_COMMAND_HANDLER(xtensa_cmd_perfmon_dump_do, target_to_xtensa(&xtensa_mcore->cores_targets[i]));
        if (res != ERROR_OK) {
            return res;
        }
    }
...
COMMAND_HANDLER(xtensa_mcore_cmd_tracestart)
{
    ...
    for (int i = 0; i < xtensa_mcore->configured_cores_num; i++) {
        int res = CALL_COMMAND_HANDLER(xtensa_cmd_tracestart_do, target_to_xtensa(&xtensa_mcore->cores_targets[i]));
        if (res != ERROR_OK) {
            return res;
        }
    }
...
COMMAND_HANDLER(xtensa_mcore_cmd_tracestop)
{
    ...
    for (int i = 0; i < xtensa_mcore->configured_cores_num; i++) {
        int res = CALL_COMMAND_HANDLER(xtensa_cmd_tracestop_do, target_to_xtensa(&xtensa_mcore->cores_targets[i]));
        if (res != ERROR_OK) {
            return res;
        }
    }
...
COMMAND_HANDLER(xtensa_mcore_cmd_tracedump)
{
    ...
    if (CMD_ARGC < xtensa_mcore->configured_cores_num) {
        command_print(CMD_CTX, "Need %d filenames to dump to as output!", xtensa_mcore->configured_cores_num);
        return ERROR_FAIL;
    }

    for (int i = 0; i < xtensa_mcore->configured_cores_num; i++) {
        int res = CALL_COMMAND_HANDLER(xtensa_cmd_tracedump_do, target_to_xtensa(&xtensa_mcore->cores_targets[i]), CMD_ARGV[i]);
        if (res != ERROR_OK) {
            return res;
        }
    }
