	...
	if (ensure_success) {
		...
		while (1) {
			...
			if (status == DMI_STATUS_BUSY) {
				...
				if (tmp != 0xFFFFFFFF) {
					increase_dmi_busy_delay(target);
					//setting *dmi_busy_encountered as true here is causing timeout. //TODO-UPS
					//if (dmi_busy_encountered)
					//	*dmi_busy_encountered = true;
				}
			} else if (status == DMI_STATUS_SUCCESS) {
		...
		uint32_t size, uint32_t count, uint8_t *buffer, uint32_t increment)
{
	...
	if (result != ERROR_OK) {
		...
		for (uint32_t i = 0; i < count; i++, address_i += increment, buffer_i += size) {
			...
			if (result != ERROR_OK) {
				LOG_TARGET_DEBUG(target, "error reading single word of %d bytes from 0x%" TARGET_PRIxADDR,
						size, address_i);
