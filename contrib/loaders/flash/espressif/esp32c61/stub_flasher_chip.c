...
/***************************************************************************
 *   ESP32-C61 specific flasher stub functions                             *
...
#define PCR_SOC_CLK_MAX                         2 // CPU_CLK frequency is 160 MHz (source is PLL_CLK)
...
uint32_t g_stub_cpu_freq_hz = CONFIG_ESP32C61_DEFAULT_CPU_FREQ_MHZ * MHZ;
...
int64_t esp_timer_get_time(void)
{
	/*
		...
		unfortunately esp32c61 does not support CPU cycle counter, so we have two options:
...
uint64_t stub_get_time(void)
{
	/* this function is used for perf measurements only.
		unfortunately esp32c61 does not support CPU cycle counter and usage of HW timer is problematic */
	return 0;
}
