...
{
	uint8_t prev_block_num = s_mem_ctrl.current_block;
	uint8_t new_block_num = !prev_block_num;
...
static int apptrace_memory_swap(void)
{
	...
	while (apptrace_memory_swap_internal() != ESP_STUB_OK) {
		...
		*/
		stub_lib_delay_us(100); /* avoid busy-loop and wait for OpenOCD to read data */
#endif
	}
...
uint8_t *apptrace_memory_uplink_get(uint32_t size)
{
	...
	if (apptrace_curr_block_pos() + apptrace_usr_block_raw_size(size) > apptrace_curr_block()->sz) {
		if (apptrace_memory_swap() != ESP_STUB_OK)
			return NULL;
	}
