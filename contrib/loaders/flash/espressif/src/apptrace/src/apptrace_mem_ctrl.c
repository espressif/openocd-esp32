...
static uint8_t *apptrace_memory_rb_produce(uint32_t size)
{
	...
	STUB_LOG_TRACEF("wr:%d rd:%d cur_size:%d rb-size:%d data-size:%d\n", rb->wr, rb->rd, rb->cur_size, rb->size, size);
...
static uint8_t *apptrace_memory_rb_consume(uint32_t size)
{
	...
	STUB_LOG_TRACEF("wr:%d rd:%d cur_size:%d size:%d\n", rb->wr, rb->rd, rb->cur_size, size);
...
{
	return s_mem_ctrl.current_block % 2;
}
...
{
	int prev_block_num = s_mem_ctrl.current_block % 2;
...
static inline uint8_t *apptrace_memory_rb_wait4buf(uint16_t size, int *pended)
{
	STUB_LOG_TRACE();

	if (apptrace_memory_swap() != 0)
		return NULL;

	if (apptrace_curr_block_pos() + size > apptrace_curr_block()->sz)
		return NULL;

	*pended = 0;
	return apptrace_curr_block()->start + apptrace_curr_block_pos();
}
...
uint8_t *apptrace_memory_downlink_get(uint32_t *size)
{
	...
	while (1) {
		uint32_t sz = apptrace_memory_rb_read_size_get();
		if (sz != 0) {
			...
			*size = MIN(*size, sz);
			ptr = apptrace_memory_rb_consume(*size);
			if (!ptr) {
...
uint8_t *apptrace_memory_uplink_get(uint32_t size)
{
	...
	if (apptrace_curr_block_pos() + apptrace_usr_block_raw_size(size) > apptrace_curr_block()->sz) {
		int pended_buf;
		STUB_LOGD(" full. Get %d bytes from pend buffer\n", size);
		buf_ptr = apptrace_memory_rb_wait4buf(apptrace_usr_block_raw_size(size), &pended_buf);
		if (buf_ptr && !pended_buf) {
			STUB_LOGD("Got %d bytes from block\n", size);
			apptrace_increment_block_pos(apptrace_usr_block_raw_size(size));
		}
	} else {
		STUB_LOGD("Get %d bytes from  buffer\n", size);
		buf_ptr = apptrace_curr_block()->start + apptrace_curr_block_pos();
		apptrace_increment_block_pos(apptrace_usr_block_raw_size(size));
	}
...
int apptrace_memory_uplink_put(uint8_t *buf)
{
	(void)buf;
