	...
	uint8_t *insn_buf, uint8_t *insn_sect, uint32_t sector_size)
{
	...
	int rc = stub_lib_flash_read_buff(sect_addr, insn_sect, inst_buff_size);
	if (rc != STUB_LIB_OK) {
	...
	rc = stub_lib_flash_erase_area(sect_addr, inst_buff_size);
	if (rc != STUB_LIB_OK) {
	...
	stub_lib_cache_invalidate_addr(bp_virt_addr, insn_sz);
...
{
	/* mp[0] carries [flash_addr, virt_addr] pairs (2 × uint32_t per BP) */
	...
	const uint8_t *insn, uint8_t *insn_sect, uint32_t sector_size)
{
	...
	for (uint8_t i = 0; i < insn_sz; i++)
		insn_sect[off + i] = insn[i];
...
static __maybe_unused int handle_flash_clock_configure(va_list ap)
{
	...
	if (new_cpu_freq == -1) {
		...
		// TODO: Has an issue with ESP32-C6
		//stub_lib_clock_init();
	}
...
int stub_main(int cmd, ...)
{
	...
	/* Must happen before flash init so that runtime settings can be preserved. */
...
out:
	...
	/* Restore MMU settings and resume suspended caches */
	stub_lib_cache_restore(cache_state);

	STUB_LOGD("stub_main ret: %d\n", ret);
