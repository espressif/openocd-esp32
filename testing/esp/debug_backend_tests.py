...
class DebuggerTestAppTests(DebuggerTestsBase):
    ...
    def run_to_bp_and_check(self, exp_rsn, func_name, lineno_var_prefs, outmost_func_name='blink_task'):
        ...
        elif re.match(r'esp32c3-[.]*', testee_info.hw_id):
            get_logger().debug('Fix backtrace for [%s]', testee_info.hw_id)
            # TODO: currently backtrace generated by riscv GDB ends with the following entries instead of FreeRTOS task wrapper
            #N-1  0x403860b4 in vPortEnterCritical ()
            #N  0x00000000 in ?? ()
            outmost_frame = len(frames) - 3
        else:
